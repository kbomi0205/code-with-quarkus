...
> 파일명 오타 하나로 한참 찾은 경험을 통해 정확한 네이밍이 얼마나 중요한지 느꼈다.
...
> 또한 Bootstrap JS 로드 순서 하나로 기능 전체가 동작하지 않는 경험을 통해 script 태그 순서와 의존성 관리가 얼마나 중요한지 깨달았다.

---

## 📌 추가 공부

> 중간고사 이후 따라가기 벅차서 수행하지 못한 과제들을 모두 수행했습니다. 13주차 PDF의 나머지 진도 부분도 추가로 구현해보았으며, 코드가 지저분해 각 페이지마다 코드를 공부하며 주석 추가 및 정리 작업을 진행했습니다.

### ✅ Week 9~13 과제 수행 내역

- **챔피언 검색 결과 모달창 연결** : `search.js` CHAMPIONS 배열에 `modalId` 추가, 검색 결과 카드 클릭 시 상세 모달 오픈
- **다크/라이트 토글 방식 변경** : 인라인 `onclick` 방식 → `addEventListener` 이벤트 리스너 방식으로 변경, 전체 HTML 파일 적용
- **다운로드 페이지 다크/라이트 모드 적용** : `download.css`에 라이트 모드 스타일 추가, `toggle.js` 연동
- **로그인 유효성 검사 구현** : `login.js`에 아이디/패스워드 정규식 검사 구현
- **프로필 사진 업로드 에러 메시지 처리** : `profile.html`에 `uploadErrorMsg` div 추가, `profile.js`에서 URL 파라미터로 에러 메시지 표시
- **Toast 알림 교체** : 기존 `alert()` 를 Bootstrap5 기반 Toast로 교체 (`main_after_login.html`, `register.html`, `register_success.html`)

### ✅ 13주차 PDF 추가 구현

- **회원정보 수정** : `profile.html`에 이메일/연락처 수정 폼 추가, `profile.js`에 유효성 검사 및 결과 메시지 처리, `AuthResource.java`에 `/profile/update` 엔드포인트 추가
- **비밀번호 변경** : `profile.html`에 비밀번호 변경 폼 추가, `profile.js`에 SHA-256 해시 처리 및 성공 Toast 후 자동 로그아웃 구현, `AuthResource.java`에 `/profile/password` 엔드포인트 추가 및 `/logout` 수정
...
| 분류 | 구현 내용 |
|------|----------|
| 프론트엔드 | LOL 테마 메인 페이지, 챔피언 카드 + 모달, 검색 기능, 다크/라이트 모드 |
| 백엔드 | Quarkus REST API, 세션 기반 로그인/로그아웃, 회원가입 |
| 보안 | SHA-256 패스워드 암호화, Forced Browsing 차단, HttpOnly 쿠키 |
| DB | MySQL 연동, JPA/Panache 엔티티, 사용자/챔피언 데이터 관리 |
| 회원관리 | 프로필 페이지, 사진 업로드, 회원정보 수정, 비밀번호 변경 |

## 🌱 성장한 점

- 단순한 HTML 페이지에서 시작해 로그인 / 회원가입 / 프로필 관리까지 갖춘 실제 웹 서비스 구조를 직접 구현해볼 수 있었습니다.
- 프론트엔드(HTML, CSS, JS)와 백엔드(Java, Quarkus)가 어떻게 연결되는지 전체 흐름을 이해하게 되었습니다.
- 단순히 코드를 따라 치는 것을 넘어, 오류가 발생했을 때 원인을 파악하고 해결하는 능력이 향상되었습니다.
- SHA-256 암호화, 세션 관리, Forced Browsing 차단 등 보안의 중요성을 실제로 체감할 수 있었습니다.

## 😅 아쉬운 점

- 중간고사 이후 수업 진도를 따라가기 벅차서 과제를 제때 수행하지 못한 점이 아쉽습니다.
- 수업 내용을 완전히 이해하지 못해 AI에게 묻는 경우가 많았고, 오류 해결도 상당 부분 AI에 의존했습니다. 덕분에 진행은 할 수 있었지만, 과연 나의 실력이 얼마나 늘었는지는 솔직히 잘 모르겠습니다.
- 뉴스, 챔피언 탭 등 미완성 링크들을 실제 데이터와 연동하는 등 추가적인 개발이 이루어지지 못한 점이 아쉽습니다.

## 💭 최종 느낀 점

> 처음 해보는 웹사이트 만들기 프로젝트라 너무나도 어려웠습니다.
> 코드 한 줄 한 줄이 낯설고 오류가 생길 때마다 막막했지만, 문제를 해결할 때마다 느끼는 뿌듯함이 상당했습니다.
> 이 수업을 통해 프론트엔드, 백엔드, 데이터베이스가 어떻게 연결되어 하나의 서비스를 이루는지 직접 경험할 수 있었고, 나만의 웹사이트를 만들어보고 싶다는 생각을 처음으로 갖게 되었습니다.
> 비록 완벽하지는 않지만, 한 학기 동안 포기하지 않고 끝까지 완성한 것에 스스로 뿌듯함을 느낍니다!!
